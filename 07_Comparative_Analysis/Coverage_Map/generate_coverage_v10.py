...
"""
==============================================================================
HyPR01 Coverage Visualization Engine (v10 - Combined Diploid Reference)
==============================================================================
Processes competitive mapping from a fully concatenated reference
(Hap1 + Hap2 + Chloroplast + Mitochondria) into publication figures and QC tables.
"""
...
def plot_nuclear(df, lengths, chroms, hap, color, haploid_ref_depth, smooth, out_stem, formats):
    """Plots nuclear chromosome tracks with embedded legend and proportional physical widths."""
    ...
    fig.suptitle(
        ...
        x=left_in / fig_w, ha="left", y=0.985, fontweight="bold", fontsize=11,
    )
...
def plot_unplaced(df, lengths, chroms, hap, color, haploid_ref_depth, out_stem, formats):
    """Plots unplaced scaffolds on a dedicated log-scale axis."""
    if not chroms:
        return
    ...
    fig.suptitle(f"HyPR01 {hap} \u2014 Unplaced Sequence Read Depth", x=0.01, ha="left", fontweight="bold")
...
def plot_hap_comparison(dists_df, chrom_order, haploid_ref_depth, out_stem, formats):
    """Renders side-by-side per-chromosome boxplots from the competitive mapping event."""
    ...
    fig.suptitle("HyPR01 Competitive Haplotype Read Partitioning (Hap1 vs Hap2)", x=0.01, ha="left", fontweight="bold")
...
def plot_organelles(df_org, lengths_org, haploid_ref_depth, geseq_gff3, out_stem, formats):
    ...
    fig.suptitle("HyPR01 Organellar Read Depth (Competitive Mapping, 200 bp windows)", x=0.01, ha="left")
...
def main():
    ...
    print("[+] STAGE 5 COMPLETE: ALL CONSOLIDATED V10 FIGURES AND TABLES GENERATED.")
