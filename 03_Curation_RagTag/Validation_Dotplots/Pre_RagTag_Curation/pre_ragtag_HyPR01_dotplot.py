...
def chr_sort_key(name):
    """
    Sorts Chromosomes numerically ('scaffold_1' / 'Chr01' -> 1) 
...
# --- PLOTTING ---
print("Generating publication-ready Pre-RagTag dotplot...")
plt.rcParams.update({'font.sans-serif': 'Arial', 'font.family': 'sans-serif'})
fig, ax = plt.subplots(figsize=(10, 10), facecolor='white')
ax.set_facecolor('white')

# 1. Grid Boundaries (Dashed Gray Lines)
for scaf in hap2_scaffolds:
...
# 2. Alignment Lines (Solid Black Synteny Line)
for aln in alignments:
    if aln['q'] in hap1_offsets and aln['t'] in hap2_offsets:
        ...
        ax.plot([x_start, x_end], [y_start, y_end], color='black', linewidth=1.0, alpha=1.0, zorder=2)

# 3. Ticks & Clean Label Formatting
...
ax.set_xlabel("Haplotype 2 (Concatenated)", fontsize=11, fontweight='normal', labelpad=12)
ax.set_ylabel("Haplotype 1 (Concatenated)", fontsize=11, fontweight='normal', labelpad=12)
ax.set_title("Synteny Dotplot: Hap1 vs Hap2 (asm20)", fontsize=14, fontweight='bold', pad=15)
...
# Save both PDF and 300 DPI PNG with solid white background
plt.savefig(out_pdf, dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig(out_png, dpi=300, bbox_inches='tight', facecolor='white')

print(f"Saved clean white vector plot: {out_pdf}")
print(f"Saved clean white raster plot: {out_png}")