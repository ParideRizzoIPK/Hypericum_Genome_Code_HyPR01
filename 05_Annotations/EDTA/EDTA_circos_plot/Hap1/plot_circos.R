...
# Generate Outputs
message("Rendering ", "Hap1", " plots...")

# Main Chromosomes Only
png(paste0("HyPR01_", "Hap1", "_TE_Landscape_MainChr.png"), width = 10, height = 10, units = "in", res = 600)
generate_circos_plot(genome_df_main)
dev.off() 

svg(paste0("HyPR01_", "Hap1", "_TE_Landscape_MainChr.svg"), width = 10, height = 10)
generate_circos_plot(genome_df_main)
dev.off()

# All Chromosomes (including unplaced contigs if present)
if (nrow(genome_df_all) > nrow(genome_df_main)) {
    png(paste0("HyPR01_", "Hap1", "_TE_Landscape_AllChr.png"), width = 10, height = 10, units = "in", res = 600)
    generate_circos_plot(genome_df_all)
    dev.off()
    
    svg(paste0("HyPR01_", "Hap1", "_TE_Landscape_AllChr.svg"), width = 10, height = 10)
    generate_circos_plot(genome_df_all)
    dev.off()
}
