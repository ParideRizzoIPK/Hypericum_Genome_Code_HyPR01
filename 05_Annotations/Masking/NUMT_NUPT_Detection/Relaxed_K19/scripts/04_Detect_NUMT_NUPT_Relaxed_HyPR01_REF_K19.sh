...
echo "########################################################################"
echo "Pipeline: Audit Control Track - Inverted Seeding Engine (K=19, W=10)"
echo "Date: $(date)"
echo "########################################################################"
...
# ==============================================================================
# MODULE R3b.1: SAFE HEREDOC ORGANELLAR FASTA CONCATENATION
# ==============================================================================
echo ">>>> CONCATENATING ORGANELLAR FASTA QUERIES VIA RAW HEREDOC... <<<<"
COMBINED_REF="${MASTER_OUT_DIR}/combined_organelles.fasta"

# FIX: Passed environmental variables explicitly as arguments to a quote-insulated Heredoc stream
python3 - "${CP_REF}" "${MT_REF}" "${COMBINED_REF}" << 'EOF'
import sys
import os
...
process_fasta(cp_input, combined_output, 'Plastid')
process_fasta(mt_input, combined_output, 'Mitochondrion')
EOF

echo "  -> Combined reference library generated at: ${COMBINED_REF}"

# ==============================================================================
# PIPELINE EXECUTION LOOP: PARALLEL CONTROL STREAMING
# ==============================================================================

# --- RUNTRACK 1: HAPLOTYPE 1 TRACK ---
HAP1_WSPACE="${MASTER_OUT_DIR}/Hap1_control_wspace"
mkdir -p "${HAP1_WSPACE}"

(
    cd "${HAP1_WSPACE}"
    echo "[$(date)] Aligning via Inverted Option Y Strategy (-k 19 -w 10)..." > r3c_hap1.log
    minimap2 -k 19 -w 10 -s 40 -c -t "${HALF_THREADS}" "${COMBINED_REF}" "${HAP1_SOFTMASKED}" > hap1_inverted_hits.paf 2> r3b_hap1.log
) &
HAP1_PID=$!

# --- RUNTRACK 2: HAPLOTYPE 2 TRACK ---
HAP2_WSPACE="${MASTER_OUT_DIR}/Hap2_control_wspace"
mkdir -p "${HAP2_WSPACE}"

(
    cd "${HAP2_WSPACE}"
    echo "[$(date)] Aligning via Inverted Option Y Strategy (-k 19 -w 10)..." > r3c_hap2.log
    minimap2 -k 19 -w 10 -s 40 -c -t "${HALF_THREADS}" "${COMBINED_REF}" "${HAP2_SOFTMASKED}" > hap2_inverted_hits.paf 2> r3b_hap2.log
) &
HAP2_PID=$!
...
with open(gff_file, 'w') as outfile:
    ...
    for r in merged_records:
        ...
        outfile.write(f"{r['seqid']}\tNUMT_NUPT_control_K19\t{r['type']}\t{r['start']}\t{r['end']}\t{r['max_score']}\t{r['strand']}\t.\t{attributes}\n")
        feature_counter += 1
...
echo "########################################################################"
echo "AUDIT CONTROL RUN COMPLETE."
echo "########################################################################"