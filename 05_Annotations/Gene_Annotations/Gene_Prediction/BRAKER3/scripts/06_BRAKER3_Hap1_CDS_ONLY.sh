...
#SBATCH --time=2-00:00:00                     # Streamlined 2-day ceiling cushion (will finish much faster)
#SBATCH --job-name=b3_cds_h1
#SBATCH --output=logs/braker3_cds_h1_%j.out
#SBATCH --error=logs/braker3_cds_h1_%j.err
...
BRAKER_OUT_DIR="${BASE_OUT}/Hap1/BRAKER3_results"
AUG_DIR="${BASE_OUT}/Hap1/augustus_config_dir"

mkdir -p "$BRAKER_OUT_DIR" "$AUG_DIR"

# Core Structural Assets (Targeting your softmasked assemblies)
GENOME="/path/to/input/directory/hap1.softmasked.fa"
PROT_DB="${BASE_OUT}/Hypericum_Comprehensive_Prot_Evidence.fasta"
CLEANED_BAM_DIR="/path/to/your/directory"

SPECIES_NAME="hypericum_perforatum_hap1_v2"
THREADS="${SLURM_CPUS_PER_TASK}"

# Compile comma-separated alignment string using the validated, purified header tracks
BAM_LIST="${CLEANED_BAM_DIR}/Bud_2012_Hap1.bam,${CLEANED_BAM_DIR}/InPetal_2012_Hap1.bam,${CLEANED_BAM_DIR}/Leaf_2012_Hap1.bam,${CLEANED_BAM_DIR}/Leaves_Hap1.bam,${CLEANED_BAM_DIR}/OutPetal_2012_Hap1.bam,${CLEANED_BAM_DIR}/Pistil_2012_Hap1.bam,${CLEANED_BAM_DIR}/Roots_Hap1.bam,${CLEANED_BAM_DIR}/Stem_2012_Hap1.bam"
...
# ==============================================================================
echo "[$(date)] Starting clean Haplotype 1 BRAKER3 CDS-only annotation run..."
